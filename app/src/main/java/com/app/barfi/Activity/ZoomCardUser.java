        ...
        mUserDatabase.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                if (!mUser.getProfileImageUrl().equals("default"))
                    Glide.with(getApplicationContext()).load(mUser.getProfileImageUrl()).thumbnail(0.1f).into(mImage);

                if (!mUser.getImageUrl().equals("default"))
                    Glide.with(getApplicationContext()).load(mUser.getImageUrl()).thumbnail(0.1f).into(mImage1);
                else mImage1.setVisibility(View.GONE);

                if (!mUser.getImageUrl3().equals("default"))
                    Glide.with(getApplicationContext()).load(mUser.getImageUrl3()).thumbnail(0.1f).into(mImage3);
